...
class UniversalLink:
    def __init__(self, host: str, database: str, user: str, password: str):
        logger.info("ESDL-AIMMS Universal link starting...")
        ...
        logger.debug(
            f"Connecting to mysql+pymysql://{user}:*****@{host},  db={database}"
        )
    ...
    def esdl_to_db(self, esdl_filename) -> Tuple[bool, str]:
        ...
        """
        logger.debug(f"Processing ESDL...")
        esh = EnergySystemHandler()
        try:
            esh.load_file(esdl_filename)
            logger.debug(f"Parsing ESDL...")
            self.parse_esdl(esh)
            return True, "Ok"
    ...
    def get_sql(self, query):
        try:
            ...
            logger.error("Error: unable to fetch data %d: %s" % (e.args[0], e.args[1]))

    def create_AIMMS_sql(self, DB, SetofTables, SetofAttributes):
        logger.debug(f"Removing and recreate database {DB}")
        ...
        try:
            query = []
            query.append(
                """CREATE TABLE `log_table` (`id` int(11) NOT NULL AUTO_INCREMENT,
            ...
            # Progress update
            logger.debug("SQL-file created from ESDL-file")
            logger.debug(query)
        except pymysql.Error as e:
            logger.error(
                "Error: unable to create table %d: %s" % (e.args[0], e.args[1])
            )

    def write_table_to_Sql(self, DB, Sheet, val):
        # Build query
        numofcol = self.get_sql(
            "SELECT COUNT(*) as NumberofCol from INFORMATION_SCHEMA.COLUMNS where table_schema = '"
            + DB
            + "' and table_name = '"
            + Sheet
            + "';"
        ...
        logger.debug("INSERT " + Sheet + " COMPLETE")
        self.conn.commit()

    def ExtractDataESDL(
        self, TableName, Instances, SetofAttributes, SetofTables, SetofValues
    ):
        if Instances == []:
            return
        valInstance = []
    ...
    def parse_esdl(self, esh, event=None, context=None):
            ...
            if len(a.port) > 1:
                for b in a.port:
                    ...
                    if type(b) == esdl.InPort:
                        ...
                        btype = "Out"
                    # logger.info(mainport)
                    # logger.info(mainport.carrier)
                    # logger.info(b)
                    if mainport.carrier != None:
                        ...
                        logger.info(
                            f"Note that process {b.id} misses attribute (carrier)"
                        )
        ...
        valCarriers = [
            (p.id, p.name, p.cost.value if p.cost and p.cost.value else None)
            for p in Carriers
        ]
        if Carriers != []:
            SetofAttributes.append(
        ...
        ElectricityCommodities = esh.get_all_instances_of_type(
            esdl.ElectricityCommodity
        )
        ...
        valCommodities = [(h.id, h.name) for h in Commodities]
        if Commodities != []:
            SetofAttributes.append(
                ("id varchar(100)  Primary Key", "name varchar(1500)")
            )
        ...
        if Matters != []:
            self.ExtractDataESDL(
                "Matters", Matters, SetofAttributes, SetofTables, SetofValues
            )
        #     Matters = esh.get_all_instances_of_type(esdl.Matter)
        #     valMatters =[]
        #     for m in Matters:
        ...
        if Buildings != []:
            ...
            valMapAssetToBuilding = [
                (b.id, b.name, a.id, a.name, "1") for a in Buildings for b in a.asset
            ]
            SetofAttributes.append(
                (
                    "id_Asset varchar(100) Primary Key",
                    "name_Asset varchar(100)",
        ...
        #     else:
        #         logger.info("KPI type: ", type(k), " is not supported")
        SetofAttributes.append(
            ("id_KPI varchar(100)", "name_KPI varchar(100)", "value_KPI varchar(100)")
        )
                ...
                if ks:
                    ...
                    for i in range(len(ks.kpi)):
                        temp = (
                            ...
                            ks.kpi[i].value
                            if type(ks.kpi[i])
                            in [esdl.IntKPI, esdl.DoubleKPI, esdl.StringKPI]
                            else None,
                            b.id,
                            b.name,
                        )
        ...
        logger.debug([c.name for c in CostInformations])
        valCostInformations = []
        for a in Assets:
            c = a.costInformation
            if a.costInformation:
                ...
                for d in dir(c):
                    e = getattr(c, d)
                    if e == None or type(e) == str or type(e) == int:
                        temp += (None,)
                    else:
                        temp += (e.value,)
                logger.debug(temp)
        ...
        for a in Assets:
            for b in a.constraint:
                ...
                # logger.info(type(b.attributeReference))
        ...
        QuantityAndUnitReferences = esh.get_all_instances_of_type(
            esdl.QuantityAndUnitReference
        )
            ...
            for p in a.port:
                for pr in p.profile:
                    ...
                    if type(pr) == esdl.QuantityAndUnitType:
                        ...
                        logger.warning(f"Profile type {type(pr)} is not supported")

        if len(QuantityAndUnitTypes) > 0:
            AssetQuantityAndUnitTypesAtt = [
        ...
        if len(InfluxDBProfiles) > 0:
            AssetQuantityAndUnitInfluxDBAtt = [
                "asset_id varchar(100)",
                "asset_name varchar(100)",
                "port_id varchar(100)",
            ]
        ...
        for a in range(len(SetofTables)):
            logger.info("Exporting:", SetofTables[a])  # , SetofValues[a])
...
def main() -> Tuple[bool, str]:
    ...
    logger.info("ESDL-AIMMS Universal link")
    logger.info(f"Processing ESDL...")
    ...
    inputfilename = "output/Tholen_Iterations_25_Time_2023-06-05T14:22:10.845513/Tholen_A=24_T=24"
    logger.info("ESDL:", "...", inputfilename)
...
if __name__ == "__main__":
    ...
    logger.error(f"Application exited with an error: {error}")
