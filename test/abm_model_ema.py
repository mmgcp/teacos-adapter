...
class Agent(mg.GeoAgent):
    ...
    def calculateFinancialFactor(self):
        PVPanelLifetime = 25
        costPerkWh = 0.50
        PVPanelYearlykWh = 375
        PVPanelCost = self.model.costOfPVPanel
        PVPanelYearlySavings = PVPanelYearlykWh * costPerkWh
    ...
    def calculateSocialFactor(self):
        self.socialFactor = sum(
            [
                a.bought
                for a in self.model.space.get_neighbors_within_distance(
                self, self.radius
            )
            ]
        ) / (
...
class TholenModel(mesa.Model):
    ...
    def __init__(
        ...
        esh.load_file("../ESDLs-Input/Tholen.esdl")
